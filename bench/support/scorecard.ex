defmodule Docket.Bench.Scorecard do
  ...
  fail is scored nil and gates the overall line. `--check` raises only on
  invariant violations, never on timing scores or not-implemented scenarios.
  """
  ...
  defp check!(results) do
    ...
    if failures != [] do
      raise "scorecard --check failed on invariant violations: #{inspect(failures)}"
    end
