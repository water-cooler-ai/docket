if Code.ensure_loaded?(Ecto.Adapters.SQL) and Code.ensure_loaded?(Postgrex) do
  defmodule Docket.Postgres.ClaimPolicy.TenantFair.RingFunctionV3 do
    @moduledoc """
    ...
    bootstrap default cap, trace flag, and literal schema ABI `3`. The eighth
    argument prevents a V3 binary from silently invoking the frozen V2
    function (and vice versa). The function returns newly claimed leases,
    ...
    blocks every later promotion in that visit. V3 never rotates queued
    discovery; it preserves the V2 continuation tuple solely for lossless
    downgrade.
    ...
    @identity_arguments "timestamp with time zone, timestamp with time zone, integer, integer, text, integer, boolean, integer"
    @lock_timeout_ms 250
    ...
    def prosrc(prefix) when is_binary(prefix) do
      ...
      BEGIN
        IF p_now IS NULL OR p_cutoff IS NULL OR p_cutoff > p_now OR
           ...
           p_trace IS NULL OR p_schema_version IS DISTINCT FROM 3 OR
          ...
          IF FOUND THEN
            ...
            SELECT COALESCE(array_agg(chosen.id ORDER BY chosen.attempt_ordinal),
                   ...
                   (SELECT eligible_at FROM ready_page
                    ORDER BY admitted, queue_ordinal DESC NULLS LAST,
                             eligible_at DESC, id DESC LIMIT 1),
            ...
            -- V3 FIFO promotion no longer rotates through queued candidates.
            -- Preserve the frozen V2 continuation tuple for a lossless down migration.
            ...
            LOOP
              ...
              IF FOUND THEN
                ...
                IF v_candidate.poison AND v_candidate_counted THEN
                  v_live_count := greatest(v_live_count - 1, 0);
            ...
            IF v_grant_count = 0 THEN
              v_disposition := CASE
                ...
                WHEN v_ready_slots = 0 AND false = ANY(v_attempt_admitted)
