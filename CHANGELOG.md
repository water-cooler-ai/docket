...
Changes targeting v0.1.0 update the Unreleased section before release.
...
- `Docket.Backend`: one backend bundle as the public durable backend
  ...
  run-aggregate, event, and supervision capabilities.
- Substrate-neutral storage ports:
...
- In-memory conformance backend exercising the full bundle contract,
  including overlapping-transaction publication (test support).
...
- The original operational transition spec and v0.1.0 spec-lock audit,
  since rewritten as a current PostgreSQL guide and
...
- `Docket.RunInfo`: token-free operational projection (`run`, `wake_at`,
  ...
  and the poisoned `await_run` typed operational halt.
...
- Processless moment entrypoints on the shared runtime loop:
  ...
  drain. Retry moments ride the durable active-superstep state: graph status
...
- Locked versioning amendment: publication materializes node schema defaults
  ...
  not durable run identity; the vehicle shell is a separate runtime concern.
...
- Postgres claim, heartbeat, release, steal, and poison operations no longer
  ...
  graph-run document while `inspect_run` reports delivery state.
...
- Postgres v01 schema finalized for the unreleased v0.1.0 contract:
  `claim_attempts` / `poisoned_at` / `poison_reason`
...
- `docket_checkpoints` table and its Ecto schema: `docket_runs.checkpoint_seq`
  ...
  history. Exactly three operational tables remain.
